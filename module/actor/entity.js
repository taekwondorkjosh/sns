...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("sns", "isPolymorphed") || false;
  }
  ...
  prepareBaseData() {
    switch ( this.data.type ) {
      ...
      case "spaceship":
        return this._prepareSpaceshipData(this.data);
    }
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.sns || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag('sns', 'transformOptions');
      const original = game.actors?.get(this.getFlag('sns', 'originalActor'));
      if (original) {
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.SNS.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.SNS.CR_EXP_LEVELS[cr];
  }

  /* -------------------------------------------- */

  /**
   * Return the amount of experience granted by defeating a ship of a certain Tier.
   * @param tier {Number}     The ship's Tier
   * @return {Number}       The amount of experience granted per ship
   */
  getTierExp(tier) {
    if (tier < 1.0) return Math.max(200 * tier, 10);
    return CONFIG.SNS.TIER_EXP_LEVELS[tier];
  }
  ...
  static async getClassFeatures({className="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.SNS.classFeatures[className];
    if (!clsConfig) return [];
  ...
  /**
   * Prepare Spaceship type specific data
   */
  _prepareSpaceshipData(actorData) {
    const data = actorData.data;

    // Kill Experience
    data.details.xp.value = this.getTierExp(data.details.tier);

    // Proficiency
    data.attributes.prof = Math.min((data.details.tier + 1), 6);
  ...
  _prepareSkills(actorData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = SNS.characterFlags;
    const athlete = flags.remarkableAthlete;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingProgression (actorData) {
    ...
    for ( let cls of classes ) {
      ...
      switch (prog) {
        ...
        case 'psion': progression.slot += Math.ceil(levels / 2); break;
        ...
        case 'battery': progression.battery += levels; break;
      }
    }
    ...
    if (!isNPC && isSingleClass && ['psion', 'half', 'third'].includes(caster.data.spellcasting) ) {
      if (caster.data.spellcasting === 'psion') {
           progression.slot = caster.data.levels;
      }
    ...
    let slots1 = SNS.SPELL_SLOT_TABLE[levels - 1] || [];
    if (caster !== null) {
        if (caster.data.spellcasting === 'psion') {
            slots1 = SNS.PSION_SPELL_SLOT_TABLE[levels - 1] || [];
        }
    }
    const slots = slots1;
    ...
    // Determine the Actor's battery magic level (if any)
    let bl = Math.clamped(progression.battery, 0, 20);
    spells.battery = spells.battery || {};
    if ( (bl === 0) && isNPC && Number.isNumeric(spells.battery.override) ) bl = actorData.data.details.spellLevel;

    // Determine the number of Dark Matter Batteries per level
    if ( bl > 0) {
      spells.battery.level = Math.ceil(Math.min(10, bl) / 2);
      if ( Number.isNumeric(spells.battery.override) ) spells.battery.max = Math.max(parseInt(spells.battery.override), 1);
      else spells.battery.max = (1 + Math.max(1, Math.min(bl-3, 2), Math.min(bl - 7, 3), Math.min(bl - 12, 4)));
      spells.battery.value = Math.min(spells.battery.value, spells.battery.max);
    } else {
      spells.battery.max = parseInt(spells.battery.override) || 0
      spells.battery.level = spells.battery.max > 0 ? 1 : 0;
    }
  }
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( this.getFlag("sns", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);
    const max = actorData.data.abilities.str.value * CONFIG.SNS.encumbrance.strMultiplier * mod;
  ...
  async update(data, options={}) {
    ...
    if ( newSize && (newSize !== getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.SNS.tokenSizes[newSize];
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("sns", "reliableTalent"));

    // Roll and return
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("SNS.SkillPromptTitle", {skill: CONFIG.SNS.skills[skillId]}),
      halflingLucky: this.getFlag("sns", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {"flags.sns.roll": {type: "skill", skillId }}
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.SNS.abilities[abilityId];
    new Dialog({
      title: game.i18n.format("SNS.AbilityPromptTitle", {ability: label}),
      content: `<p>${game.i18n.format("SNS.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("SNS.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("SNS.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const feats = this.data.flags.sns || {};
    if ( feats.remarkableAthlete && SNS.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
    ...
    const rollData = mergeObject(options, {
      ...
      messageData: {"flags.sns.roll": {type: "ability", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("SNS.SavePromptTitle", {ability: label}),
      halflingLucky: this.getFlag("sns", "halflingLucky"),
      messageData: {"flags.sns.roll": {type: "save", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("SNS.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("sns", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("SNS.DeathSavingThrow"),
      speaker: speaker,
      halflingLucky: this.getFlag("sns", "halflingLucky"),
      targetValue: 10,
      messageData: {"flags.sns.roll": {type: "death"}}
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("SNS.DeathSaveCriticalSuccess", {name: this.name}), speaker});
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({content: game.i18n.format("SNS.DeathSaveSuccess", {name: this.name}), speaker});
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        await ChatMessage.create({content: game.i18n.format("SNS.DeathSaveFailure", {name: this.name}), speaker});
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("SNS.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const title = game.i18n.localize("SNS.HitDiceRoll");
    ...
    const roll = await damageRoll({
      ...
      messageData: {"flags.sns.roll": {type: "hitDie"}}
    });
    if ( !roll ) return null;
  ...
  async shortRest({dialog=true, chat=true, autoHD=false, autoHDThreshold=3}={}) {
    ...
    if ( chat ) {
      ...
      let restFlavor;
      switch (game.settings.get("sns", "restVariant")) {
        case 'normal': restFlavor = game.i18n.localize("SNS.ShortRestNormal"); break;
        case 'gritty': restFlavor = game.i18n.localize(newDay ? "SNS.ShortRestOvernight" : "SNS.ShortRestGritty"); break;
        case 'epic':  restFlavor = game.i18n.localize("SNS.ShortRestEpic"); break;
      }

      // Summarize the health effects
      let srMessage = "SNS.ShortRestResultShort";
      if ((dhd !== 0) && (dhp !== 0)) srMessage = "SNS.ShortRestResult";
  ...
  async longRest({dialog=true, chat=true, newDay=true}={}) {
    ...
    // Recover battery charges.
    const battery = data.spells.battery;
    updateData['data.spells.battery.value'] = battery.override || 0;
    ...
    let restFlavor;
    switch (game.settings.get("sns", "restVariant")) {
      case 'normal': restFlavor = game.i18n.localize(newDay ? "SNS.LongRestOvernight" : "SNS.LongRestNormal"); break;
      case 'gritty': restFlavor = game.i18n.localize("SNS.LongRestGritty"); break;
      case 'epic':  restFlavor = game.i18n.localize("SNS.LongRestEpic"); break;
    }

    // Determine the chat message to display
    if ( chat ) {
      let lrMessage = "SNS.LongRestResultShort";
      if((dhp !== 0) && (dhd !== 0)) lrMessage = "SNS.LongRestResult";
      else if ((dhp !== 0) && (dhd === 0)) lrMessage = "SNS.LongRestResultHitPoints";
      else if ((dhp === 0) && (dhd !== 0)) lrMessage = "SNS.LongRestResultHitDice";
      ChatMessage.create({
        user: game.user._id,
        speaker: {actor: this, alias: this.name},
        flavor: restFlavor,
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("sns", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("SNS.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = duplicate(this.toJSON());
    o.flags.sns = o.flags.sns || {};
    o.flags.sns.transformOptions = {mergeSkills, mergeSaves};
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: 'class',
        name: game.i18n.localize('SNS.PolymorphTmpClass'),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.sns.originalActor ) d.flags.sns.originalActor = this.id;
    d.flags.sns.isPolymorphed = true;
    ...
    Hooks.callAll('sns.transformActor', this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.owner ) {
      return ui.notifications.warn(game.i18n.localize("SNS.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: 'SNS.PolymorphRestoreTransformation',
