...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.SNS.areaTargetTypes);
  }
  ...
  prepareData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("SNS.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.ship) ) labels.featType = game.i18n.localize("SNS.ShipActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("SNS.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "SNS.Attack" : "SNS.Action");
      else labels.featType = game.i18n.localize("SNS.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("SNS.AC")}` : "";
      labels.activated = data.propertiesofitems.activated ? `${game.i18n.localize("SNS.ItemEquipmentActivated")}` : "";
      labels.cover = data.propertiesofitems.cover ? `${game.i18n.localize("SNS.Cover")}` : "";
      labels.networkenabled = data.propertiesofitems.networkenabled ? `${game.i18n.localize("SNS.ItemEquipmentNetworkEnabled")}` : "";
      labels.resistballistic = data.propertiesofitems.resistballistic ? `${game.i18n.localize("SNS.ItemEquipmentResistBallistic")}` : "";"";
      labels.spaceable = data.propertiesofitems.spaceable ? `${game.i18n.localize("SNS.ItemEquipmentSpaceable")}` : "";
      labels.thrusters = data.propertiesofitems.thrusters ? `${game.i18n.localize("SNS.ItemEquipmentThrusters")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("SNS.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.SNS.abilities[save.ability];
    this.labels.save = game.i18n.format("SNS.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.SNS.spellUpcastModes.includes(id.preparation.mode);
	...
	let spellLevel1 = parseInt(slotLevel);
	if (slotLevel === "pact") {
		spellLevel1 = actor.data.data.spells.pact.level;
	}
	if (slotLevel === "battery") {
		spellLevel1 = actor.data.data.spells.battery.level;
	}
	const spellLevel = spellLevel1
        ...
        if ( consumeSpellSlot ) {
		if (slotLevel === "pact") {	
			consumeSpellSlot = "pact";
		}
		else {
			consumeSpellSlot = `spell${spellLevel}`;
		}
		if (slotLevel === "battery") {
			consumeSpellSlot = "battery";
		}
		else {
			consumeSpellSlot = `spell${spellLevel}`;
		}
  ...
  _getUsageUpdates({consumeQuantity=false, consumeRecharge=false, consumeResource=false, consumeSpellSlot=false, consumeUsage=false}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("SNS.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellSlot ) {
      ...
      if ( spells === 0 ) {
        let label1 = game.i18n.localize(consumeSpellSlot === "pact" ? "SNS.SpellProgPact" : `SNS.SpellLevel${id.level}`);
	if (consumeSpellSlot === "battery") {
		label1 = game.i18n.localize("SNS.SpellProgBattery");
	}
	const label = label1;
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.SNS.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("SNS.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( !resource ) {
      ui.notifications.warn(game.i18n.format("SNS.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("SNS.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const template = `systems/sns/templates/chat/${templateType}-card.html`;
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["sns.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.SNS.attunementTypes.REQUIRED ) props.push(game.i18n.localize(CONFIG.SNS.attunements[CONFIG.SNS.attunementTypes.REQUIRED]));
      props.push(
        game.i18n.localize(data.equipped ? "SNS.Equipped" : "SNS.Unequipped"),
        game.i18n.localize(data.proficient ? "SNS.Proficient" : "SNS.NotProficient"),
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.SNS.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.propertiesofitems.strdexdis.value ? game.i18n.localize("SNS.StrDexDisav") : null,
      data.cover.value ? game.i18n.localize("SNS.Cover") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.SNS.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.SNS.consumableTypes[data.consumableType],
      data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("SNS.Charges")
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.SNS.abilities[data.ability] || null,
      CONFIG.SNS.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("SNS.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("SNS.AbbreviationLbs") : null
    );
  }
  ...
  async rollAttack(options={}) {
    ...
    const rollConfig = mergeObject({
      ...
      messageData: {"flags.sns.roll": {type: "attack", itemId: this.id }}
    }, options);
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.sns.roll": {type: "damage", itemId: this.id }};
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "SNS.Healing" : "SNS.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.sns.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("sns", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("SNS.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.sns.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("SNS.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "SNS.ItemRechargeSuccess" : "SNS.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("SNS.ToolCheck")}`;

    // Compose the roll data
    const rollConfig = mergeObject({
      parts: parts,
      data: rollData,
      template: "systems/sns/templates/chat/tool-roll-dialog.html",
      title: title,
      speaker: ChatMessage.getSpeaker({actor: this.actor}),
      flavor: title,
      dialogOptions: {
      ...
      halflingLucky: this.actor.getFlag("sns", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("sns", "reliableTalent"),
      messageData: {"flags.sns.roll": {type: "tool", itemId: this.id }}
    }, options);
    rollConfig.event = options.event;
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("sns", "itemData");
    const item = storedData ? this.createOwned(storedData, actor) : actor.getOwnedItem(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("SNS.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}))
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("SNS.ActionWarningNoToken"));
    return targets;
  }
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.SNS.spellScrollIds[level];
    ...
    const spellScrollData = mergeObject(scrollData, {
      name: `${game.i18n.localize("SNS.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
