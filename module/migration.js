...
export const migrateWorld = async function() {
  ...
  for ( let a of game.actors.entities ) {
    try {
      ...
      err.message = `Failed sns system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items.entities ) {
    try {
      ...
      err.message = `Failed sns system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes.entities ) {
    try {
      ...
      err.message = `Failed sns system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("sns", "systemMigrationVersion", game.system.data.version);
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let ent of content ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed sns system migration for entity ${ent.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.SNS.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.sns ) {
    actorData.flags.sns = filterObject(actorData.flags.sns, allowedFlags);
  }
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.SNS.attunementTypes.NONE;
...
export async function purgeFlags(pack) {
  const cleanFlags = (flags) => {
    const flags5e = flags.sns || null;
    return flags5e ? {sns: flags5e} : {};
  };
  await pack.configure({locked: false});
  const content = await pack.getContent();
