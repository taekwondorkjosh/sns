/**
 * The SnS game system for Foundry Virtual Tabletop
 ...
 * Repository: https://gitlab.com/foundrynet/sns
 * Issue Tracker: https://gitlab.com/foundrynet/sns/issues
 */

// Import Modules
import { SNS } from "./module/config.js";
...
import ActorSheet5eSpaceship from "./module/actor/sheets/spaceship.js";
...
Hooks.once("init", function() {
  console.log(`SnS | Initializing the SnS Game System\n${SNS.ASCII}`);
  ...
  Actors.registerSheet("sns", ActorSheet5eCharacter, {
    types: ["character"],
    makeDefault: true,
    label: "SNS.SheetClassCharacter"
  });
  Actors.registerSheet("sns", ActorSheet5eNPC, {
    types: ["npc"],
    makeDefault: true,
    label: "SNS.SheetClassNPC"
  });
  Actors.registerSheet("sns", ActorSheet5eSpaceship, {
    types: ["spaceship"],
    makeDefault: true,
    label: "SNS.SheetClassSpaceship"
  });
  Actors.registerSheet('sns', ActorSheet5eVehicle, {
    types: ['vehicle'],
    makeDefault: true,
    label: "SNS.SheetClassVehicle"
  });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("sns", ItemSheet5e, {
    makeDefault: true,
    label: "SNS.SheetClassItem"
  });
...
Hooks.once("setup", function() {
  ...
  for ( let o of toLocalize ) {
    const localized = Object.entries(CONFIG.SNS[o]).map(e => {
      return [e[0], game.i18n.localize(e[1])];
    });
    if ( !noSort.includes(o) ) localized.sort((a, b) => a[1].localeCompare(b[1]));
    CONFIG.SNS[o] = localized.reduce((obj, e) => {
      obj[e[0]] = e[1];
      return obj;
    }, {});
  }
...
Hooks.once("ready", function() {
  ...
  const currentVersion = game.settings.get("sns", "systemMigrationVersion");
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("sns", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("sns", "autoCollapseItemCards")) html.find(".card-content").hide();
});
